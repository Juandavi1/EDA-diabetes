...
diabetes <- glucose %>%
  ...
  select(-HeartRate.x, -HeartRate.y)
...
```{r}
preProcessDiabetes <- preProcess(diabetes, method = c("center", "scale"))
...
Se aplica la funcion prcomp para determinar los componenten principales 
del dataset.
...
La función fviz_cos2 tiene como objetivo visualizar las contribuciones de las variables originales a los componentes principales generados por el PCA.

```{r}
ggplotly(fviz_cos2(PCA, choice = "var", axes = 1:2))
```

Según el grafico, las variables IMC, peso y altura son las más importantes para explicar la variabilidad de los datos. 
...
Uno de los principales factores de diabetes es el sobre peso, 
ya que gracias a ello, el cuerpo genera resistencia a la insulina 
lo que conduce a un aumento en los niveles de azúcar en sangre.

Por lo cual, el grupo que contenga el mayor indice de sobre peso, 
será el seleccionado como el grupo vulnerable. 

El limite entre sobre peso y obesidad es 30. Por lo cual, 
el grupo que acumule más pacientes superior a este rango, 
indicará que ahí se encuentran los pacientes vulnerables a diabetes. 

Según el siguiente gráfico, el grupo vulnerable es el 2. 
